 ...
 *
 * Eigenfaces variant: kicks off async data load (Faces.load) before mounting scenes,
 * so any scene that touches mean/eigenfaces/projections can rely on window.FACES being ready. */

(function () {
  const SCENE_TITLES = [
    "",                                // 0: title hero
    "Meet the cast",
    "A face is just numbers",
    "The mean face",
    "Subtract the mean",
    "The eigenfaces",
    "Reconstruction",
  ...
  async function init() {
    ...
    // Load binary blobs in the background. Scenes can also call Faces.ready()
    // themselves if they want a fine-grained hook.
    if (window.Faces && Faces.load) {
      Faces.load().catch(err => console.error('Faces.load failed:', err));
    }
